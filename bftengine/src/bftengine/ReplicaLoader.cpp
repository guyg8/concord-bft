...
ReplicaLoader::ErrorCode loadConfig(PersistentStorage* p, LoadedReplicaData& ld) {
	...
	ld.repConfig = p->getReplicaConig();
...
ReplicaLoader::ErrorCode checkViewDesc(
	const PersistentStorage::DescriptorOfLastExitFromView* exitDesc,
	const PersistentStorage::DescriptorOfLastNewView* newDesc)
{
...
ReplicaLoader::ErrorCode loadViewInfo(PersistentStorage* p, LoadedReplicaData& ld) {
	...
	PersistentStorage::DescriptorOfLastExitFromView descriptorOfLastExitFromView;
	PersistentStorage::DescriptorOfLastNewView descriptorOfLastNewView;
	...
	LoadedReplicaData& ld)
{
	...
	for (size_t i = 0; i < sizeof(ld.checkWinArr) / sizeof(LoadedReplicaData::CheckWinData); i++)
	{
		...
		if (seqNum <= ld.lastExecutedSeqNum)
		{
	...
	if (p->hasDescriptorOfLastExecution()) {
		PersistentStorage::DescriptorOfLastExecution d = p->getDescriptorOfLastExecution();
		if (d.executedSeqNum != ld.lastExecutedSeqNum) { 
			...
			Verify(numOfReqs > 0, InconsistentErr);
