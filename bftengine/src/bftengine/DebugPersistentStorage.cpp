...
void DebugPersistentStorage::setLastViewThatTransferredSeqNumbersFullyExecuted(const ViewNum v)
{
	Assert(nonExecSetIsAllowed());
...
void DebugPersistentStorage::setDescriptorOfLastExitFromView(const DescriptorOfLastExitFromView & d)
{
	...
	for (size_t i = 0; i < d.elements.size(); i++)
	{
		...
		PrePrepareMsg* clonedPrePrepareMsg = (PrePrepareMsg*)e.prePrepare->cloneObjAndMsg();
	...
	descriptorOfLastExitFromView_ = DescriptorOfLastExitFromView{ d.view, d.lastStable, d.lastExecuted , clonedElements };
}
...
void DebugPersistentStorage::setDescriptorOfLastNewView(const DescriptorOfLastNewView & d)
{
	...
	descriptorOfLastNewView_ = DescriptorOfLastNewView{ d.view, clonedNewViewMsg, clonedViewChangeMsgs, d.maxSeqNumTransferredFromPrevViews };
}
...
void DebugPersistentStorage::setDescriptorOfLastExecution(const DescriptorOfLastExecution & d)
{
	...
	// TODO(GG): check size of d.validRequests
...
void DebugPersistentStorage::setPrePrepareMsgInSeqNumWindow(const SeqNum s, const PrePrepareMsg * const m)
{
...
void DebugPersistentStorage::setSlowStartedInSeqNumWindow(const SeqNum s, const bool slowStarted)
{
	Assert(seqNumWindow.insideActiveWindow(s));
	SeqNumData& seqNumData = seqNumWindow.get(s);
...
void DebugPersistentStorage::setFullCommitProofMsgInSeqNumWindow(const SeqNum s, const FullCommitProofMsg * const m)
{
	Assert(seqNumWindow.insideActiveWindow(s));
	SeqNumData& seqNumData = seqNumWindow.get(s);
...
void DebugPersistentStorage::setForceCompletedInSeqNumWindow(const SeqNum s, const bool forceCompleted)
{
...
void DebugPersistentStorage::setPrepareFullMsgInSeqNumWindow(const SeqNum s, const PrepareFullMsg * const m)
{
	Assert(seqNumWindow.insideActiveWindow(s));
	SeqNumData& seqNumData = seqNumWindow.get(s);
...
void DebugPersistentStorage::setCommitFullMsgInSeqNumWindow(const SeqNum s, const CommitFullMsg * const m)
{
	Assert(seqNumWindow.insideActiveWindow(s));
	SeqNumData& seqNumData = seqNumWindow.get(s);
...
void DebugPersistentStorage::setCheckpointMsgInCheckWindow(const SeqNum s, const CheckpointMsg * const m)
{
	Assert(checkWindow.insideActiveWindow(s));
	CheckData& checkData = checkWindow.get(s);
	if (checkData.checkpointMsg != nullptr)
		delete checkData.checkpointMsg;
	checkData.checkpointMsg = (CheckpointMsg*)m->cloneObjAndMsg();
}

void DebugPersistentStorage::setCompletedMarkInCheckWindow(const SeqNum s, const bool f)
{
	Assert(f == true);
	Assert(checkWindow.insideActiveWindow(s));
	CheckData& checkData = checkWindow.get(s);
...
bool DebugPersistentStorage::hasReplicaConfig()
{
	return hasConfig_;
}
...
PersistentStorage::DescriptorOfLastExitFromView DebugPersistentStorage::getAndAllocateDescriptorOfLastExitFromView()
{
	Assert(getIsAllowed());
	Assert(hasDescriptorOfLastExitFromView_);
	...
	DescriptorOfLastExitFromView retVal{ d.view, d.lastStable , d.lastExecuted , elements };
...
PersistentStorage::DescriptorOfLastNewView DebugPersistentStorage::getAndAllocateDescriptorOfLastNewView()
{
	Assert(getIsAllowed());
	...
	DescriptorOfLastNewView retVal{ d.view, newViewMsg, viewChangeMsgs, d.maxSeqNumTransferredFromPrevViews };
...
PersistentStorage::DescriptorOfLastExecution DebugPersistentStorage::getDescriptorOfLastExecution()
{
	Assert(getIsAllowed());
	...
	return PersistentStorage::DescriptorOfLastExecution{ d.executedSeqNum, d.validRequests };
}
...
PrePrepareMsg * DebugPersistentStorage::getAndAllocatePrePrepareMsgInSeqNumWindow(const SeqNum s)
{
	Assert(getIsAllowed());
	Assert(lastStableSeqNum_ + 1 == seqNumWindow.currentActiveWindow().first);
	Assert(seqNumWindow.insideActiveWindow(s));
	PrePrepareMsg* m = (PrePrepareMsg*) seqNumWindow.get(s).prePrepareMsg->cloneObjAndMsg();
...
FullCommitProofMsg * DebugPersistentStorage::getAndAllocateFullCommitProofMsgInSeqNumWindow(const SeqNum s)
{
	Assert(getIsAllowed());
	Assert(lastStableSeqNum_ + 1 == seqNumWindow.currentActiveWindow().first);
	Assert(seqNumWindow.insideActiveWindow(s));
	FullCommitProofMsg* m = (FullCommitProofMsg*)seqNumWindow.get(s).fullCommitProofMsg->cloneObjAndMsg();
...
PrepareFullMsg * DebugPersistentStorage::getAndAllocatePrepareFullMsgInSeqNumWindow(const SeqNum s)
{
	Assert(getIsAllowed());
	Assert(lastStableSeqNum_ + 1 == seqNumWindow.currentActiveWindow().first);
	Assert(seqNumWindow.insideActiveWindow(s));
	PrepareFullMsg* m = (PrepareFullMsg*)seqNumWindow.get(s).prepareFullMsg->cloneObjAndMsg();
	Assert(m->type() == MsgCode::PrepareFull);
	return m;
}

CommitFullMsg * DebugPersistentStorage::getAndAllocateCommitFullMsgInSeqNumWindow(const SeqNum s)
{
	Assert(getIsAllowed());
	Assert(lastStableSeqNum_ + 1 == seqNumWindow.currentActiveWindow().first);
	Assert(seqNumWindow.insideActiveWindow(s));
	CommitFullMsg* m = (CommitFullMsg*)seqNumWindow.get(s).commitFullMsg->cloneObjAndMsg();
	Assert(m->type() == MsgCode::CommitFull);
	return m;
}

CheckpointMsg * DebugPersistentStorage::getAndAllocateCheckpointMsgInCheckWindow(const SeqNum s)
{
	Assert(getIsAllowed());
	Assert(lastStableSeqNum_ == checkWindow.currentActiveWindow().first);
	Assert(checkWindow.insideActiveWindow(s));
	CheckpointMsg* m = (CheckpointMsg*)checkWindow.get(s).checkpointMsg->cloneObjAndMsg();
...
void DebugPersistentStorage::WindowFuncs::free(SeqNumData& i)
{
	reset(i);
}
...
void DebugPersistentStorage::WindowFuncs::free(CheckData& i)
{
	reset(i);
}
