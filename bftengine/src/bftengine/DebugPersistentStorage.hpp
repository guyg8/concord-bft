...
public:
	...
	virtual void setLastViewThatTransferredSeqNumbersFullyExecuted(const ViewNum v) override;
	virtual void setDescriptorOfLastExitFromView(const DescriptorOfLastExitFromView & prevViewDesc) override;
	virtual void setDescriptorOfLastNewView(const DescriptorOfLastNewView & prevViewDesc) override;
	virtual void setDescriptorOfLastExecution(const DescriptorOfLastExecution & prevViewDesc) override;
	virtual void setLastStableSeqNum(const SeqNum s) override;
	virtual void clearSeqNumWindow() override;
	virtual void setPrePrepareMsgInSeqNumWindow(const SeqNum s, const PrePrepareMsg * const m) override;
	virtual void setSlowStartedInSeqNumWindow(const SeqNum s, const bool slowStarted) override;
	virtual void setFullCommitProofMsgInSeqNumWindow(const SeqNum s, const FullCommitProofMsg * const m) override;
	virtual void setForceCompletedInSeqNumWindow(const SeqNum s, const bool forceCompleted) override;
	virtual void setPrepareFullMsgInSeqNumWindow(const SeqNum s, const PrepareFullMsg * const m) override;
	virtual void setCommitFullMsgInSeqNumWindow(const SeqNum s, const CommitFullMsg * const m) override;
	virtual void setCheckpointMsgInCheckWindow(const SeqNum s, const CheckpointMsg * const m) override;
	virtual void setCompletedMarkInCheckWindow(const SeqNum s, const bool f) override;
	...
	virtual DescriptorOfLastExitFromView getAndAllocateDescriptorOfLastExitFromView() override;
	virtual bool hasDescriptorOfLastNewView() override;
	virtual DescriptorOfLastNewView getAndAllocateDescriptorOfLastNewView() override;
	...
	virtual PrePrepareMsg * getAndAllocatePrePrepareMsgInSeqNumWindow(const SeqNum s) override;
	virtual bool getSlowStartedInSeqNumWindow(const SeqNum s) override;
	virtual FullCommitProofMsg * getAndAllocateFullCommitProofMsgInSeqNumWindow(const SeqNum s) override;
	virtual bool getForceCompletedInSeqNumWindow(const SeqNum s) override;
	virtual PrepareFullMsg * getAndAllocatePrepareFullMsgInSeqNumWindow(const SeqNum s) override;
	virtual CommitFullMsg * getAndAllocateCommitFullMsgInSeqNumWindow(const SeqNum s) override;
	virtual CheckpointMsg * getAndAllocateCheckpointMsgInCheckWindow(const SeqNum s) override;
...
protected:
	...
	DescriptorOfLastExitFromView descriptorOfLastExitFromView_ = DescriptorOfLastExitFromView{ 0, 0, 0, std::vector<ViewsManager::PrevViewInfo>(0) };
	bool hasDescriptorOfLastNewView_ = false;
	DescriptorOfLastNewView descriptorOfLastNewView_ = DescriptorOfLastNewView{ 0 , nullptr, std::vector<ViewChangeMsg*>(0), 0 };
	...
	SequenceWithActiveWindow<kWorkWindowSize, 1, SeqNum, SeqNumData, WindowFuncs> seqNumWindow;

	// range: TODO(GG): !!!!!!!
	SequenceWithActiveWindow<kWorkWindowSize + checkpointWindowSize, checkpointWindowSize, SeqNum, CheckData, WindowFuncs> checkWindow;
